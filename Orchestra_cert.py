...
#font
fontpath = "font/GenRyuMin-SB.ttc"
font = ImageFont.truetype(fontpath, 270)

#check eligibility
concert_one = pd.read_csv("record/20211227.csv")
concert_two = pd.read_csv("record/20220315.csv")
...
if id == "20211227期末音樂會":
    if text_input in concert_one.values:
        generate_cert(filename = 'asset/20211227.png')
    elif text_input:
    ...
    if text_input in concert_two.values:
        generate_cert(filename = "asset/20220315.png")
    elif text_input:
        st.write("%s您好，"%(text_input))
        st.write("您並未參與本場音樂會擔任志工")
        st.write("若有問題歡迎來信tmuorchestra1979@gmail.com")